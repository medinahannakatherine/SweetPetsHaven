...
if (isset($_POST['submit'])) {
  ...
  $author =  $_POST['author'];
  $testi_id = $_SESSION['testi_id_edit'];
  $updateData = [
  ...
  $fetch_data = $database->getReference($ref_table)->orderByChild('testi_id')->equalTo((int) $_SESSION['testi_id_edit'])->getValue();
...
<body>
  ...
  <div class="container mt-5">
    <div class='col-12 text-center'>
      <h2 class="mb-5">Edit Testimonial</h2>
    </div>
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>"
                  ...
                  foreach ($fetch_data as $key => $row) {
                    ...
                    <input class="form-control mb-3" type="text" id="title" name="title"
                      value="<?php echo $row['testi_name']; ?>" required>
                      <label for="aut" class="form-label">Author</label>
                    <input class="form-control mb-3" type="text" id="aut" name="author"
                      value="<?php echo $row['testi_author']; ?>" required>
                    <div class="mb-3">
                      <label for="description" class="form-label">Description</label>
                      <div class="">
                        <textarea class="form-control" name="description" id="description" rows="5" style="resize: none;"
                          value="<?php echo $row['testi_description']; ?>" required><?php echo $row['testi_description']; ?></textarea>
                        ...
                        <div class="text-center">
                          ...
                          <?php
                          $get_data = $database->getReference('testimonial_pic')->orderByChild('testi_id')->equalTo((int) $_SESSION['testi_id_edit'])->getValue();
                          if ($get_data > 0) {
                    ...
                    <div class="text-center">
                      <button type="submit" class="btn-secondary" name="submit">Edit Testimonial</button>
                    </div>
                  </form>
                <?php } ?>
              </div>
